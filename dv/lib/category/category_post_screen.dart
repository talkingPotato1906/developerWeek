...
class _CategoryPostScreenState extends State<CategoryPostScreen> {
  ...
  try {
    await _firestore.runTransaction((transaction) async {
      ...
      // 읽은 데이터 처리
      int currentReactions = (postSnapshot.data() as Map<String, dynamic>)["reactions"] ?? 0;
      List<dynamic> reactedUsers = (postSnapshot.data() as Map<String, dynamic>)["reacted"] ?? [];
                    ...
                    children: [
                      ...
                      TextButton(
                        onPressed: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(builder: (context) => OthersGalleryScreen(othersUid: postData!["uid"]),)
                          );
                        },
                        child: Text(
                          userData!["nickname"],
                    ...
                    children: [
                      GestureDetector(
                        ...
                        child: Icon(
                          Icons.favorite,
                          color:
                              _isLiked ? Colors.red : Colors.grey, // 누르면 빨간색
                        ),
                      ),
                      SizedBox(
                        width: 5,
